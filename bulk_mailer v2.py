...
class BulkMailerApp:
    ...
    def create_widgets(self):
        ...
        # Signature Selector
        ttk.Label(frame, text="Select Signature:").grid(row=7, column=0, sticky=tk.W)
        self.signature_selector = ttk.Combobox(frame)
        self.signature_selector.grid(row=7, column=1, sticky=tk.W)
        self.load_signatures()

        ttk.Button(frame, text="Send Emails", command=self.send_emails).grid(row=8, column=0, columnspan=2, sticky=tk.E)

    def load_signatures(self):
        signature_dir = os.path.join(os.environ['USERPROFILE'], 'AppData\\Roaming\\Microsoft\\Signatures')
        self.signature_files = [f for f in os.listdir(signature_dir) if f.endswith('.htm')]
        if not self.signature_files:
            messagebox.showwarning("No Signatures Found", "No Outlook signatures found in the default location.")
        self.signature_selector['values'] = self.signature_files
    ...
    def send_emails(self):
        ...
        selected_signature = self.signature_selector.get()
        if not selected_signature:
            messagebox.showwarning("Missing Signature", "Please select an email signature.")
            return
        ...
        # Assuming signature files are in default location, adjust if necessary
        signature_dir = os.path.join(os.environ['USERPROFILE'], 'AppData\\Roaming\\Microsoft\\Signatures')
        signature_html_path = os.path.join(signature_dir, selected_signature)
